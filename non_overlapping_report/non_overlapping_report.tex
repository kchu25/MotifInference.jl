...
The key finding turns on \emph{how you measure}. Counting every nonzero activation, the operator
barely seems to separate filters. But motif finding does not act on every activation --- it acts
only on \emph{strong} ones (magnitude above the $90$th percentile). Measured that way, the picture
is favorable: with the operator off, the strong activations are already fairly separated, and
sharpening the softmax strength $\alpha$ separates them almost completely --- by $\alpha\approx20$,
four-fifths of active positions are private to a single filter --- at almost no cost to held-out
accuracy. The operator is off by default and, when off, leaves every forward output
bitwise-identical.
\end{abstract}
...
different filters fire at different positions'' just counts how many filters are active at each
position. Crucially, ``active'' is defined the way motif finding defines it: a filter is active
at a position when its magnitude exceeds the \texttt{activation\_thresh} percentile (here the
$90$th) of the activation distribution --- the same global magnitude threshold
BanzhafInference applies before enumerating motifs. So the metric counts only the
\emph{strong} activations that discovery actually acts on, not every nonzero. Let
$A\in\{0,1\}^{S\times K}$ mark those thresholded activations and $d_s=\sum_k A_{s,k}$ the number
of filters active at position $s$. Over occupied positions ($d_s>0$):
\begin{itemize}
...
\end{itemize}
This is not an arbitrary metric: it measures, at the activation level, the very property the
downstream discovery counter enforces at the combination level. EpicHyperSketch (v0.0.8) counts a
co-occurring motif group only when its filters are non-overlapping --- it discards any group whose
consecutive filters have gap $<\texttt{filter\_len}$ (here $\texttt{filter\_len}=\texttt{pfm\_len}=1$,
so the requirement is simply ``distinct positions''). Occupancy and exclusivity are the
embedding-level reading of exactly that ``distinct positions'' condition.
...
the model.
\input{results/exp1_table.tex}
\input{results/exp1_interp.tex}
...
discovery dataframes, without HTML rendering. Note that the counter already only reports
\emph{non-overlapping} groups (\S\ref{sec:disc}), so what changes here is \emph{how many} such
groups survive, not whether they overlap.
\input{results/exp2_table.tex}
\input{results/exp2_interp.tex}

\subsection{Experiment 3 --- softmax strength vs.\ separation vs.\ generalization}
Experiment~1 showed the plain operator ($\alpha=1$) already nudges the strong activations apart.
How far can we push it --- does turning up the softmax strength $\alpha$ approach
one-filter-per-position, and what does it cost in held-out accuracy? We sweep
$\alpha\in\{1,5,20,100\}$ against an operator-off baseline. Every model shares the same
initialization and a fixed $40$-epoch schedule, trained on $80\%$ of the sequences; we report
occupancy on the held-out $20\%$ and held-out DMS accuracy (Spearman $\rho$, Pearson $r$).
\input{results/exp3_table.tex}
\input{results/exp3_interp.tex}
